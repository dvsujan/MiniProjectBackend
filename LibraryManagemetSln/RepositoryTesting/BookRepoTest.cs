...
namespace RepositoryTestProj
{
    public class Tests
    {
        ...
        [Test]
        public async Task DeleteBookThrowsEntityNotFoundException()
        {
            Assert.ThrowsAsync<EntityNotFoundException>(() => BookRepo.Delete(0));
        }
        ...
        public async Task GetBookByTitle()
        {
            ...
            await BookRepo.Insert(book);
            var result = await ((BookRepository)BookRepo).GetBookByTitle(book.Title);
            Assert.IsNotNull(result);
        }
        
        [Test]
        public async Task GetPaginatedBooks()
        {
            var context = new LibraryManagementContext(optionsBuilder.Options);
            var book = new Book
            {
            ...
            await BookRepo.Insert(book);
            var result = await ((BookRepository)BookRepo).GetPaginated(1, 1);
            Assert.IsNotNull(result);
        }
